...
public class Main {
    ...
    public static boolean performAction(ServiceCenter serviceCenter, int _option, Scanner inputScan){
        ...
        switch(_option){
            case 1:
                ...
                while(flag)
                {   //use try catch to deal with variable type input error 
                    try
                    {
                        String triageLevel = getInput(inputScan, "Please enter the triage level of patient: ");
                        while(Integer.parseInt(triageLevel) > 5 || Integer.parseInt(triageLevel) < 1)
                        {
                            triageLevel = getInput(inputScan, "Sorry, it is out of triage level, please enter the accepted triage level (1-5) of patient: ");//use while loop to handle the exception for out of triage level range
                        }
                    ...
                    {
                        System.out.println("Sorry, the input is invalid, please enter accepted integer number again");
                    }
                }
                break;
            case 2:
            ...
            case 5:
                clearScreen();
                printSubHeader("Check the position by id");
    ...
    public static void main(String[] args) {
        ...
        while(_continue){
            try
            {   //use try catch to handle invalid input option
                displayMenu();
            ...
            {
                System.out.println("Sorry, the input option is invalid, please enter valid integer number");
            }
        }
    }
}
